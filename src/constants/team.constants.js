export const teamConstants = {
    GETALL_REQUEST: 'TEAM_GETALL_REQUEST',
    GETALL_SUCCESS: 'TEAM_GETALL_SUCCESS',
    GETALL_FAILURE: 'TEAM_GETALL_FAILURE',

    GETALL_MEMBERS_REQUEST: 'TEAM_GETALL_MEMBERS_REQUEST',
    GETALL_MEMBERS_SUCCESS: 'TEAM_GETALL_MEMBERS_SUCCESS',
    GETALL_MEMBERS_FAILURE: 'TEAM_GETALL_MEMBERS_FAILURE',

    GET_REQUEST: 'TEAM_GET_REQUEST',
    GET_SUCCESS: 'TEAM_GET_SUCCESS',
    GET_FAILURE: 'TEAM_GET_FAILURE',

    CREATE_REQUEST: 'TEAM_CREATE_REQUEST',
    CREATE_SUCCESS: 'TEAM_CREATE_SUCCESS',
    CREATE_FAILURE: 'TEAM_CREATE_FAILURE',

    UPDATE_REQUEST: 'TEAM_UPDATE_REQUEST',
    UPDATE_SUCCESS: 'TEAM_UPDATE_SUCCESS',
    UPDATE_FAILURE: 'TEAM_UPDATE_FAILURE',

    DELETE_REQUEST: 'TEAM_DELETE_REQUEST',
    DELETE_SUCCESS: 'TEAM_DELETE_SUCCESS',
    DELETE_FAILURE: 'TEAM_DELETE_FAILURE'
};
